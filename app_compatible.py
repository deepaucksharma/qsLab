# app_compatible.py - Learning App Backend with optional TTS support
...
try:
    ...
    print("To enable TTS, use Python 3.9-3.11 and install: pip install TTS")
    ...
    while True:
        ...
        try:
            ...
            if TTS_AVAILABLE and tts:
                ...
                # Mock response when TTS not available
                generation_status[task_id] = {
                    'status': 'completed',
                    'progress': 100,
                    'mock': True,
                    'text': task['text'],
                    'message': 'Audio generation not available - TTS not installed'
                }
        except Exception as e:
            generation_status[task_id] = {
                'status': 'error',
...
if __name__ == '__main__':
    print("\n" + "="*60)
    print("Neural Learn - AI Learning App")
    print("="*60)
    if TTS_AVAILABLE:
        print("✅ TTS is available - Full audio features enabled")
    else:
        print("⚠️  TTS not available - Running in text-only mode")
        print("   To enable audio, use Python 3.9-3.11")
