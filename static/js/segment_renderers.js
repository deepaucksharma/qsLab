...
class SegmentRenderers {
    constructor() {
        ...
        // Initialize LLM visual renderer if available
        this.llmVisualRenderer = window.LLMVisualRenderer ? new window.LLMVisualRenderer() : null;
        
        // Track if we should generate LLM visuals
        this.enableLLMVisuals = true;
    ...
    async render(segment) {
        // Check if we should generate LLM visuals for this segment
        if (this.enableLLMVisuals && this.shouldEnhanceWithLLMVisual(segment)) {
            await this.generateLLMVisual(segment);
        }
    ...
    shouldEnhanceWithLLMVisual(segment) {
        // Segment types that benefit from LLM visual enhancement
        const visualSegmentTypes = [
            'technical_introduction', 'architecture_design',
            'metric_deep_dive', 'new_metric_deep_dive', 'metrics_overview',
            'historical_context', 'origin_story',
            'code_walkthrough', 'practical_example', 'practical_configuration',
            'paradigm_shift', 'technology_comparison', 'decision_framework',
            'scalability_concept', 'concept_explanation'
        ];
        
        return visualSegmentTypes.includes(segment.segmentType) && 
               !segment.llmVisualGenerated; // Don't regenerate if already done
    }
    
    async generateLLMVisual(segment) {
        if (!this.llmVisualRenderer) return;
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (result.success && result.specification) {
                    // Store the specification in the segment
                    segment.llmVisualSpec = result.specification;
                    segment.llmVisualGenerated = true;
                }
            }
        } catch (error) {
            console.error('Failed to generate LLM visual:', error);
        }
    }
    ...
    renderVisualAssets(segment) {
        ...
        // First check for LLM-generated visual
        if (segment.llmVisualSpec && this.llmVisualRenderer) {
            const llmVisualContainer = document.createElement('div');
            llmVisualContainer.className = 'llm-visual-container';
            llmVisualContainer.id = `llm-visual-${segment.segmentId}`;
            
            // Render the LLM visual asynchronously
            setTimeout(() => {
                this.llmVisualRenderer.renderVisual(
                    segment.llmVisualSpec, 
                    llmVisualContainer
                );
            }, 0);
            
            visuals.push(`<div class="segment-visual segment-visual-center">${llmVisualContainer.outerHTML}</div>`);
        }
        
        // Then check for traditional visual assets
        if (this.visualAssetManager) {
